...
const sjf = (arrivalTime: number[], burstTime: number[]) => {
...
export default function SJFPage() {
  ...
  const handleSolve = async () => {
    try {
      ...
      const arrivalArray = processes
        .map((p) => Number.parseFloat(p.arrivalTime))
        .filter((n) => !isNaN(n));
      const burstArray = processes
        .map((p) => Number.parseFloat(p.burstTime))
        .filter((n) => !isNaN(n));
      ...
      if (
        ...
        arrivalArray.length !== processes.length
      ) {
      ...
      const result = sjf(arrivalArray, burstArray);
      setResults(result);
      setIsCalculating(false);
    } catch (error) {
  ...
  const GanttChart = ({
    ...
    const uniqueTimes = [
      ...new Set([
        0,
        ...ganttChartInfo.flatMap((item) => [item.start, item.stop]),
      ]),
    ].sort((a, b) => a - b);
          ...
          <div className="flex border border-gray-300 dark:border-gray-600 rounded overflow-hidden mb-4">
            {ganttChartInfo.map((item, index) => (
              <div
                ...
                } text-white text-center py-4 px-2 font-semibold flex items-center justify-center transition-all duration-300 hover:opacity-80`}
                style={{
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 transition-colors duration-300">
              ...
              {results ? (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-lg p-4 text-center">
                      ...
                      <p className="text-xl font-bold text-emerald-600">
                        {(
                          results.solvedProcessesInfo.reduce(
                            (sum, p) => sum + p.tat,
                            0
                ...
                <div className="flex flex-col items-center justify-center h-64 text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-center text-sm">
                    Add your processes and click "Solve Algorithm" to see the
                    SJF scheduling visualization.
                  </p>
                </div>
              )}
            </CardContent>
      ...
      <div className="fixed bottom-6 right-6 z-50">
        <Button
          ...
          className="w-12 h-12 rounded-full bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700 text-white shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
          size="sm"
        >
              ...
              <div className="text-center space-y-4">
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    AlgoFlow • SJF Algorithm Visualizer
                  </p>
                </div>
              </div>
            </div>
